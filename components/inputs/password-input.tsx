    ...
    <InputWrapper {...props}>
      <div className="relative mt-2 rounded-md shadow-sm flex ring-1 ring-inset focus:ring-inset focus:ring-primary focus:ring-2">
        <input
          ...
          disabled={disabled}
          className={twMerge(
            'z-0 block w-full rounded-l-md border-0 p-1.5 shadow-sm  ring-surface-5 bg-surface placeholder:text-tc-3 text-tc-1 focus:ring-2  sm:text-sm sm:leading-6',
            hasError &&
        ...
        >
          {isVisible ? (
            <EyeIcon className="h-5 w-5 text-tc-3" aria-hidden="true" />
          ) : (
            <EyeSlashIcon className="h-5 w-5 text-tc-3" aria-hidden="true" />
          )}
        </IconButton>
      </div>
    </InputWrapper>
